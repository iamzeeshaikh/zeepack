...
export default async function LocationPage({
  ...
  return (
    <>
      ...
      {/* Genuinely local detail — the part of the page that is not templated.
          Only rendered where it has actually been written. */}
      {localContent && (
        <section className="section-space pt-0">
          <Container>
            <Reveal>
              <SectionHeading
                eyebrow={`On the ground in ${location.name}`}
                title={`What we ship to ${location.name}.`}
              />
            </Reveal>
        ...
        <Container>
          <Reveal>
            <SectionHeading
              ...
              description={`These answers cover what ${location.name} businesses ask most often about local delivery and the formats their industries use.`}
            />
          </Reveal>
          <div className="mt-10 max-w-4xl">
            <FAQAccordion items={faqs} />
            <p className="mt-6 text-sm text-[var(--color-muted)]">
              Looking for minimums, lead times, materials, or artwork requirements?{" "}
